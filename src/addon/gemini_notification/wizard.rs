use super::{HookEvent, InstallOptions, NotifMethod, GEMINI_CATEGORIES};
use anyhow::Result;
...
pub fn visible_indices(events: &[EventOpt], search: &str, active_tab: usize) -> Vec<usize> {
    ...
    events
        ...
        .enumerate()
        .filter_map(|(i, e)| {
            let tab_match = active_tab == 0
                || GEMINI_CATEGORIES.get(active_tab - 1) == Some(&e.event.category());
...
pub fn next_state(state: State, key: KeyCode) -> State {
    match (state, key) {
        ...
        (State::SelectMethod { .. }, KeyCode::Enter) => State::SelectEvents {
            events: EventOpt::all_events(),
            cursor: 0,
            search: String::new(),
            active_tab: 0,
        },
        (State::SelectMethod { .. }, KeyCode::Esc) => State::Cancelled,
        ...
        (
            ...
            let n_tabs = GEMINI_CATEGORIES.len() + 1;
...
fn render(f: &mut ratatui::Frame, state: &State) {
    ...
    let block = Block::default()
        .title(" tlink · install gemini-notification ")
        .borders(Borders::ALL)
    ...
    match state {
            ...
            f.render_widget(
                Paragraph::new(vec![
                    Line::from(Span::styled(
                        "gemini-notification add-on",
                        Style::default().add_modifier(Modifier::BOLD),
                    )),
                    Line::from(""),
                    Line::from("When Gemini CLI finishes a task or needs your attention,"),
                    Line::from("you'll get a desktop notification showing the tmux location."),
                    Line::from(""),
                    Line::from("This wizard will configure:"),
                    Line::from("  • Notification delivery method"),
                    Line::from("  • Which Gemini events trigger a notification"),
                    Line::from(""),
                    Line::from("Installed via hooks in:"),
                    Line::from("  ~/.gemini/settings.json"),
                    Line::from(""),
                    Line::from(Span::styled(
        ...
        State::SelectEvents {
            ...
            let tab_spans: Vec<Span> = std::iter::once("All".to_string())
                .chain(GEMINI_CATEGORIES.iter().map(|c| c.to_string()))
                .enumerate()
                .flat_map(|(i, label)| {
        ...
        State::Confirm { method, events } => {
            ...
            f.render_widget(
                Paragraph::new(vec![
                    ...
                    Line::from(
                        "  Hook script        : ~/.config/tlink/hooks/gemini-notification.sh"
                            .to_string(),
                    ),
                    Line::from("  Gemini settings    : ~/.gemini/settings.json".to_string()),
                    Line::from(""),
                    Line::from(Span::styled(
...
mod tests {
    ...
    #[test]
    fn test_defaults_after_agent_checked() {
        let events = EventOpt::all_events();
        let get = |ev: &HookEvent| events.iter().find(|e| &e.event == ev).unwrap().selected;
        assert!(get(&HookEvent::AfterAgent));
        assert!(!get(&HookEvent::SessionStart));
    }
    ...
    fn test_select_events_enter_collects_selected() {
        ...
        if let State::Confirm { events, .. } = next {
            assert!(events.contains(&HookEvent::AfterAgent));
        } else {
    ...
    fn test_confirm_enter_goes_to_installing() {
        let state = State::Confirm {
            method: NotifMethod::Osascript,
            events: vec![HookEvent::AfterAgent],
        };
        assert!(matches!(
