...
fn build_deeplink(session: &str, window: &str, pane: &str, term: &str, socket: &str) -> String {
    ...
    let params: Vec<String> = [
        (!term.is_empty()).then(|| {
            ...
            format!("term={}", percent_encode(term_name))
        }),
        // Skip the default socket: `tmux -L default` == no `-L`, so links for
        // the common case stay byte-for-byte unchanged.
        (!socket.is_empty() && socket != "default")
            .then(|| format!("socket={}", percent_encode(socket))),
    ]
    .into_iter()
    .flatten()
    .collect();
